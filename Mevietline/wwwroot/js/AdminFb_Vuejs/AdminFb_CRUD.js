Admin_fb = new Vue({
    ...
    mounted() {
        axios.get("/AdminFb/HoSoXuLyRut")
            .then((response) => {
                this.userRut = response.data;
                return Promise.resolve();
            })
            .then(() => {
                $("#hoSo_xuLyRut").DataTable({
                    'columnDefs': [{
                        'targets': [-1],
                        'orderable': false,
                    }],
        ...
        axios.get("/adminfb/HoSoXuLyData")
            .then((response) => {
                this.userNap = response.data;
                return Promise.resolve();
            })
            .then(() => {
                $("#hoSo_xuLy").DataTable({
    ...
    methods: {
        async handleRut(userRut) {
            const formData = new FormData();
            try {
                const response = await axios.get('/Home/GetDataProfileMainTien'); // Gọi API để lấy dữ liệu
                this.tienDangCo = response.data[0].tienDangCo; // Cập nhật giá trị vào biến tienDangCo
            } catch (error) {
                console.log(error);
            }
            curentThis = this;
            if (!userRut.trangThaiGiaoDich) {
                userRut.trangThaiGiaoDich = true;
                formData.append('TrangThaiGiaoDich', true);
                formData.append('SoTienRut', userRut.soTienRut);
                formData.append('UserId', userRut.userId);
                formData.append('Id', userRut.id);

                axios.post('/AdminFb/CapNhatLenhRut', formData, {
                    headers: {
        ...
        async AddNewItems() {
            const { value: formValues } = await Swal.fire({
                ...
                html:
                    ...
                    '<label> Số lượng tài khoản</label>' +
                    '<input v-model="SoLuongTaiKhoan" name="SoLuongTaiKhoan" id="SoLuongTaiKhoan" class="swal2-input" type="number" placeholder="Nhập số lượng">',
            ...
            if (formValues) {
                const [NguoiBan, giaCa, SoLuongTaiKhoan] = formValues;
                currentThis = this;
                const formData = new FormData();
        ...
        async handleTransaction(userNap) {
            ...
            curentThis = this;
            if (!userNap.trangThaiGiaoDich) {
                userNap.trangThaiGiaoDich = true;
                formData.append('TrangThaiGiaoDich', true);
                formData.append('SoTienNap', userNap.soTienNap);
                formData.append('UserIDInfo', userNap.userIDInfo);
                formData.append('ID', userNap.id);

                axios.post('/AdminFb/CapNhatLenh', formData, {
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded'
                    }
